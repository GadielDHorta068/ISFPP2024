...
public class ResourceExtractor {
    ...
    /**
     * Método principal que llama a la extracción de recursos.
     * 
    ...
    /**
     * Extrae los archivos de recursos al directorio de ejecución.
     */
    public static void extractResourcesToExecutionDir() {
        // Obtiene el directorio desde donde se está ejecutando el jar
        ...
        for (String file : filesToExtract) {
            try (InputStream in = Objects.requireNonNull(ResourceExtractor.class.getClassLoader().getResourceAsStream(file))) {
                ...
                // Copia el archivo al directorio de ejecución
                Files.copy(in, Paths.get(executionDir, file), StandardCopyOption.REPLACE_EXISTING);
                System.out.println("Archivo extraído: " + file);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
