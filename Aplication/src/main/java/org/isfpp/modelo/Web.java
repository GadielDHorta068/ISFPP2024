...
public class Web {
	...
	//agrega un equipo, sin que venga un puerto y su cantidad
	public Equipment addEquipment(String code, String description, String marca, String model,
								  EquipmentType equipmentType, Location location,Boolean status)  {
		if (hardware.containsKey(code))
			throw new AlredyExistException("el equipo ya se encuentra");
		if(equipmentTypes.containsKey(equipmentType))
			throw new NotFoundException("El tipo de equipo no se encuentra en la lista");
		Equipment newEquipment = new Equipment(code, description, marca, model, equipmentType,
				location,status);
		hardware.put(code, newEquipment);
		return newEquipment;
	}
